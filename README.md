...
The `inference_data` folder includes five raw samples for each modality, which can be used for model inference. Inference of the GCN model can be performed using these data together with the _pretrained model_ available on the [Google drive](https://drive.google.com/drive/folders/1_4G-9qhwgZsm_UMOA2BGuL-4l0MZdWin?usp=drive_link). 
...
./run_snf_LATENT.sh
```
To generate the patient similarity graph for the inference dataset, simply specify the dataset path using the `-infer_p` argument.
