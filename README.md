...
The `sampled_data` folder includes five raw samples for each modality, which can be used for model validation. GCN inference can be performed using these data together with the pretrained model available on the [Google drive](https://drive.google.com/drive/folders/1_4G-9qhwgZsm_UMOA2BGuL-4l0MZdWin?usp=drive_link).
